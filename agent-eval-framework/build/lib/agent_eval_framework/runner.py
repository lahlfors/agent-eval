...
def run_evaluation(config_path: str):
    ...
    for index, record in df_dataset.iterrows():
         ...
         if not prompt:
             logger.warning(f"Missing 'prompt' in record {index}")
         ...
         try:
             ...
             logger.error(f"Adapter failed for prompt '{prompt}': {e}", exc_info=True)
    ...
    # *** Send results via standard logging ***
    try:
        # Convert summary_metrics (which may contain non-serializable objects) to a serializable format
        summary_metrics_serializable = {}
        if eval_result.summary_metrics:
            for k, v in eval_result.summary_metrics.items():
                if hasattr(v, 'to_dict'):
                    summary_metrics_serializable[k] = v.to_dict()
                else:
                    summary_metrics_serializable[k] = v

        eval_payload = {
            "event_type": "evaluation_result",
            "experiment_name": config.get("experiment_name"),
            "experiment_run_name": eval_result.experiment_run_name,
            "dataset_path": config.get("dataset_path"),
            "agent_adapter_class": config.get("agent_adapter_class"),
            "summary_metrics": summary_metrics_serializable,
            "metrics_table": eval_result.metrics_table.to_dict(orient='records')
        }
        logger.info(json.dumps(eval_payload))
        print("Evaluation results sent to OTel logging pipeline.")
    except Exception as e:
        logger.error(f"Error sending eval results to OTel: {e}", exc_info=True)
